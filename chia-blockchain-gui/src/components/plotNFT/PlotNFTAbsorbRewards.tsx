...
import { pupps_to_chia, chia_to_pupps } from '../../util/chia';
...
export default function PlotNFTAbsorbRewards(props: Props) {
  ...
  async function handleSubmit(data: SubmitData) {
    try {
      ...
      const feePuppss = chia_to_pupps(fee);

      if (walletId === undefined || !address) {
        return;
      }

      await dispatch(pwAbsorbRewards(walletId, feePuppss));
          ...
          >
            <Typography variant="subtitle1">
              <Trans>
                You will recieve{' '}
                <UnitFormat
                  value={pupps_to_chia(BigInt(balance))}
                  display="inline"
                  state={State.SUCCESS}
                />{' '}
                to {wallet?.address}
