...
import { pupps_to_chia_string, chia_to_pupps } from '../../../util/chia';
  ...
  return (
    <Grid item xs={12}>
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">
            {pupps_to_chia_string(props.balance)} {currencyCode}
          </Typography>
        </Box>
      </Box>
    </Grid>
...
const SendCard = (props) => {
  ...
  function send() {
    ...
    const amount = chia_to_pupps(amount_input.value);
    const fee = chia_to_pupps(fee_input.value);
