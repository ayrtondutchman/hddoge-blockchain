...
from hddogepos import DiskPlotter

from hddoge.consensus.coinbase import create_puzzlehash_for_pk
from hddoge.plotting.plot_tools import stream_plot_info_ph, stream_plot_info_pk
from hddoge.protocols import farmer_protocol
from hddoge.rpc.farmer_rpc_api import FarmerRpcApi
from hddoge.rpc.farmer_rpc_client import FarmerRpcClient
from hddoge.rpc.harvester_rpc_api import HarvesterRpcApi
from hddoge.rpc.harvester_rpc_client import HarvesterRpcClient
from hddoge.rpc.rpc_server import start_rpc_server
...
from hddoge.wallet.derive_keys import master_sk_to_wallet_sk
...
class TestRpc:
    ...
    async def test1(self, simulation):
        ...
        try:
            ...
            await client.set_reward_targets(encode_puzzle_hash(new_ph, "hdg"), encode_puzzle_hash(new_ph_2, "hdg"))
            ...
            await client.set_reward_targets(None, encode_puzzle_hash(new_ph_3, "hdg"))
            ...
            assert config["farmer"]["hdg_target_address"] == encode_puzzle_hash(new_ph, "hdg")
            assert config["pool"]["hdg_target_address"] == encode_puzzle_hash(new_ph_3, "hdg")

            new_ph_3_encoded = encode_puzzle_hash(new_ph_3, "hdg")
