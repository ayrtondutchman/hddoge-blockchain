...
from hddoge.protocols.shared_protocol import protocol_version
from hddoge.server.outbound_message import NodeType
from hddoge.server.server import HddogeServer, ssl_context_for_client
from hddoge.server.ws_connection import WSHddogeConnection
from hddoge.ssl.create_ssl import generate_ca_signed_cert
from hddoge.types.peer_info import PeerInfo
from tests.block_tools import test_constants
from hddoge.util.ints import uint16
...
async def establish_connection(server: HddogeServer, dummy_port: int, ssl_context) -> bool:
...
class TestSSL:
    ...
    async def test_public_connections(self, wallet_node):
        ...
        server_1: HddogeServer = full_node_api.full_node.server
    ...
    async def test_farmer(self, harvester_farmer):
        ...
        generate_ca_signed_cert(
            farmer_server.hddoge_ca_crt_path.read_bytes(), farmer_server.hddoge_ca_key_path.read_bytes(), pub_crt, pub_key
        )
        ssl_context = ssl_context_for_client(
            farmer_server.hddoge_ca_crt_path, farmer_server.hddoge_ca_crt_path, pub_crt, pub_key
        )
        connected = await establish_connection(farmer_server, 12312, ssl_context)
    ...
    async def test_full_node(self, wallet_node):
        ...
        generate_ca_signed_cert(
            full_node_server.hddoge_ca_crt_path.read_bytes(),
            full_node_server.hddoge_ca_key_path.read_bytes(),
            pub_crt,
            pub_key,
        )
        ssl_context = ssl_context_for_client(
            full_node_server.hddoge_ca_crt_path, full_node_server.hddoge_ca_crt_path, pub_crt, pub_key
        )
    ...
    async def test_wallet(self, wallet_node):
        ...
        generate_ca_signed_cert(
            wallet_server.hddoge_ca_crt_path.read_bytes(), wallet_server.hddoge_ca_key_path.read_bytes(), pub_crt, pub_key
        )
        ssl_context = ssl_context_for_client(
            wallet_server.hddoge_ca_crt_path, wallet_server.hddoge_ca_crt_path, pub_crt, pub_key
        )
    ...
    async def test_harvester(self, harvester_farmer):
        ...
        generate_ca_signed_cert(
            harvester_server.hddoge_ca_crt_path.read_bytes(),
            harvester_server.hddoge_ca_key_path.read_bytes(),
            pub_crt,
            pub_key,
        )
        ssl_context = ssl_context_for_client(
            harvester_server.hddoge_ca_crt_path, harvester_server.hddoge_ca_crt_path, pub_crt, pub_key
        )
    ...
    async def test_introducer(self, introducer):
        ...
        # Create not authenticated cert
        pub_crt = introducer_server.hddoge_ca_key_path.parent / "p2p.crt"
        pub_key = introducer_server.hddoge_ca_key_path.parent / "p2p.key"
        generate_ca_signed_cert(
            introducer_server.hddoge_ca_crt_path.read_bytes(),
            introducer_server.hddoge_ca_key_path.read_bytes(),
            pub_crt,
            pub_key,
        )
        ssl_context = ssl_context_for_client(
            introducer_server.hddoge_ca_crt_path, introducer_server.hddoge_ca_crt_path, pub_crt, pub_key
        )
    ...
    async def test_timelord(self, timelord):
        ...
        generate_ca_signed_cert(
            timelord_server.hddoge_ca_crt_path.read_bytes(),
            timelord_server.hddoge_ca_key_path.read_bytes(),
            pub_crt,
            pub_key,
        )
        ssl_context = ssl_context_for_client(
            timelord_server.hddoge_ca_crt_path, timelord_server.hddoge_ca_crt_path, pub_crt, pub_key
        )
