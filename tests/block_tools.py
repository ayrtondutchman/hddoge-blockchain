...
from hddoge.cmds.init_funcs import create_all_ssl, create_default_hddoge_config
from hddoge.full_node.bundle_tools import (
    best_solution_generator_from_template,
    detect_potential_template_generator,
    simple_solution_generator,
)
from hddoge.plotting.create_plots import create_plots
from hddoge.consensus.block_creation import create_unfinished_block, unfinished_block_to_full_block
from hddoge.consensus.block_record import BlockRecord
from hddoge.consensus.blockchain_interface import BlockchainInterface
from hddoge.consensus.coinbase import create_puzzlehash_for_pk
from hddoge.consensus.constants import ConsensusConstants
from hddoge.consensus.default_constants import DEFAULT_CONSTANTS
from hddoge.consensus.deficit import calculate_deficit
from hddoge.consensus.full_block_to_block_record import block_to_block_record
from hddoge.consensus.make_sub_epoch_summary import next_sub_epoch_summary
from hddoge.consensus.pot_iterations import (
    calculate_ip_iters,
...
from hddoge.consensus.vdf_info_computation import get_signage_point_vdf_info
from hddoge.full_node.signage_point import SignagePoint
from hddoge.plotting.plot_tools import PlotInfo, load_plots, parse_plot_info
...
from hddoge.types.blockchain_format.proof_of_space import ProofOfSpace
from hddoge.types.blockchain_format.sized_bytes import bytes32
from hddoge.types.blockchain_format.slots import (
...
from hddoge.types.generator_types import BlockGenerator, CompressorArg
...
from hddoge.util.block_cache import BlockCache
from hddoge.util.config import load_config, save_config
from hddoge.util.hash import std_hash
from hddoge.util.ints import uint8, uint32, uint64, uint128
from hddoge.util.keychain import Keychain, bytes_to_mnemonic
from hddoge.util.path import mkdir
from hddoge.util.vdf_prover import get_vdf_info_and_proof
from tests.wallet_tools import WalletTool
from hddoge.wallet.derive_keys import (
...
class BlockTools:
    ...
    def __init__(
        ...
        create_default_hddoge_config(root_path)
        ...
        if len(self.pool_pubkeys) == 0 or len(farmer_pubkeys) == 0:
            raise RuntimeError("Keys not generated. Run `hddoge generate keys`")
    ...
    def init_plots(self, root_path: Path):
        ...
        try:
            ...
            args.pool_contract_address = encode_puzzle_hash(self.pool_ph, "hdg")
...
def get_plot_dir() -> Path:
    cache_path = Path(os.path.expanduser(os.getenv("HDDOGE_ROOT", "~/.hddoge/"))) / "test-plots"
    mkdir(cache_path)
    return cache_path
