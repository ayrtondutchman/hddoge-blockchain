...
from hddoge.daemon.server import WebSocketServer, create_server_for_daemon, daemon_launch_lock_path, singleton
from hddoge.full_node.full_node_api import FullNodeAPI
from hddoge.server.start_farmer import service_kwargs_for_farmer
from hddoge.server.start_full_node import service_kwargs_for_full_node
from hddoge.server.start_harvester import service_kwargs_for_harvester
from hddoge.server.start_introducer import service_kwargs_for_introducer
from hddoge.server.start_service import Service
from hddoge.server.start_timelord import service_kwargs_for_timelord
from hddoge.server.start_wallet import service_kwargs_for_wallet
from hddoge.simulator.start_simulator import service_kwargs_for_full_node_simulator
from hddoge.timelord.timelord_launcher import kill_processes, spawn_process
...
from hddoge.util.ints import uint16, uint32
...
async def setup_farmer(
    ...
    config["hdg_target_address"] = encode_puzzle_hash(b_tools.farmer_ph, "hdg")
    config["pool_public_keys"] = [bytes(pk).hex() for pk in b_tools.pool_pubkeys]
    config["port"] = port
    config_pool["hdg_target_address"] = encode_puzzle_hash(b_tools.pool_ph, "hdg")
