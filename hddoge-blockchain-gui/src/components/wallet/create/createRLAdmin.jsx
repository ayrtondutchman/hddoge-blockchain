...
import { AlertDialog } from '@hddoge/core';
import {
  Typography,
  Button,
  Box,
...
import { hddoge_to_mojo } from '../../../util/hddoge';
...
export const CreateRLAdminWallet = () => {
  ...
  function create() {
    ...
    if (
      hddogeper_input.value === '' ||
      Number(hddogeper_input.value) === 0 ||
      !Number(hddogeper_input.value) ||
      isNaN(Number(hddogeper_input.value))
    ) {
      dispatch(
        openDialog(
          <AlertDialog>
    ...
    const hddogeper = hddoge_to_mojo(hddogeper_input.value);
    const hddogeper_value = Number.parseInt(Number(hddogeper));
    ...
    // var fee = hddoge_to_mojo(fee_input.value);
