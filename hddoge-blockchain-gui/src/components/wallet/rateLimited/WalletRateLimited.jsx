...
import { AlertDialog, Card, Flex } from '@hddoge/core';
import {
  send_transaction,
...
const IncompleteCard = (props) => {
  ...
  function submit() {
    ...
    const hddogeper_input = ip_parsed.limit;
    ...
    const hddogeper_value = Number.parseInt(Number(hddogeper_input));
        ...
        <Grid item xs={12}>
          <Box display="flex">
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>
                  Spending Limit (hddoge per interval):{' '}
                  {mojo_to_hddoge_string(limit)}
                </Trans>
              </Typography>
            </Box>
          </Box>
