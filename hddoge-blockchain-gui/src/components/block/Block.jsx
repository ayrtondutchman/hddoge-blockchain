...
import { Card, FormatLargeNumber, Link, Loading, TooltipIcon, Flex } from '@hddoge/core';
import {
  unix_to_short_date,
  hex_to_array,
  arr_to_hex,
...
export default function Block() {
  ...
  const poolReward = mojo_to_hddoge(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = mojo_to_hddoge(calculateBaseFarmerReward(blockRecord.height));

  const hddogeFees = blockRecord.fees
    ? mojo_to_hddoge(BigInt(blockRecord.fees))
    : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: (
        <Link target="_blank" href={`https://www.hddogeexplorer.com/blockchain/puzzlehash/${blockRecord.farmer_puzzle_hash}`}>
    ...
    },
    {
      ...
      value: (
        <Link target="_blank" href={`https://www.hddogeexplorer.com/blockchain/puzzlehash/${blockRecord.pool_puzzle_hash}`}>
    ...
    },
    {
      ...
      value: hddogeFees ? `${hddogeFees} ${currencyCode}` : '',
      tooltip: (
        <Trans>
    ...
    >
      <Card
        title={(
          <BlockTitle>
            <Trans>
              Block at height {blockRecord.height} in the Hddoge
              blockchain
            </Trans>
          </BlockTitle>
        )}
