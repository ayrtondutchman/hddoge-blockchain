...
import hddogeEnvironment from '../util/hddogeEnvironment';
import hddogeConfig from '../util/config';
...
if (!handleSquirrelEvent()) {
  ...
  const ensureCorrectEnvironment = () => {
    ...
    if (!hddogeEnvironment.guessPackaged() && !('VIRTUAL_ENV' in process.env)) {
  ...
  if (ensureSingleInstance() && ensureCorrectEnvironment()) {
    ...
    const createWindow = () => {
      ...
      if (!hddogeConfig.manageDaemonLifetime()) {
      ...
      mainWindow.on('close', (e) => {
        ...
        if (decidedToClose || !hddogeConfig.manageDaemonLifetime()) {
          return;
        }
        e.preventDefault();
        if (!isClosing) {
    ...
    const appReady = async () => {
      ...
      // if the daemon isn't local we aren't going to try to start/stop it
      if (hddogeConfig.manageDaemonLifetime()) {
        hddogeEnvironment.startHddogeDaemon();
      }
    };
  ...
  const getMenuTemplate = () => {
    ...
    if (process.platform === 'darwin') {
      // Hddoge Blockchain menu (Mac)
      template.unshift({
        label: i18n._(/* i18n */ { id: 'Hddoge' }),
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'About Hddoge Blockchain' }),
            click: () => {
              openAbout();
            },
          },
