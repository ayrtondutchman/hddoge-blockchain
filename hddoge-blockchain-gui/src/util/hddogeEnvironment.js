...
const getHddogeVersion = () => {
  let version = null;
  const exePath = getExecutablePath('hddoge');
  // first see if we can get a hddoge exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or hddoge is on the path
    try {
      version = child_process
