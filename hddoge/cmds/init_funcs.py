...
from hddoge.ssl.create_ssl import generate_ca_signed_cert, get_hddoge_ca_crt_key, make_ca_cert
from hddoge.util.bech32m import encode_puzzle_hash
from hddoge.util.config import (
    create_default_hddoge_config,
    initial_config_file,
    load_config,
    save_config,
...
from hddoge.wallet.derive_keys import master_sk_to_pool_sk, master_sk_to_wallet_sk
...
def check_keys(new_root: Path) -> None:
    ...
    if len(all_sks) == 0:
        print("No keys are present in the keychain. Generate them with 'hddoge keys generate'")
        return None

    config: Dict = load_config(new_root, "config.yaml")
    pool_child_pubkeys = [master_sk_to_pool_sk(sk).get_g1() for sk, _ in all_sks]
    all_targets = []
    stop_searching_for_farmer = "hdg_target_address" not in config["farmer"]
    stop_searching_for_pool = "hdg_target_address" not in config["pool"]
    ...
    for i in range(number_of_ph_to_search):
        ...
        for sk, _ in all_sks:
            ...
            if all_targets[-1] == config["farmer"].get("hdg_target_address"):
                stop_searching_for_farmer = True
            if all_targets[-1] == config["pool"].get("hdg_target_address"):
                stop_searching_for_pool = True

    # Set the destinations
    if "hdg_target_address" not in config["farmer"]:
        print(f"Setting the hdg destination address for coinbase fees reward to {all_targets[0]}")
        config["farmer"]["hdg_target_address"] = all_targets[0]
    elif config["farmer"]["hdg_target_address"] not in all_targets:
        print(
            f"WARNING: using a farmer address which we don't have the private"
            f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
            f"{config['farmer']['hdg_target_address']} with {all_targets[0]}"
        )

    if "pool" not in config:
        config["pool"] = {}
    if "hdg_target_address" not in config["pool"]:
        print(f"Setting the hdg destination address for coinbase reward to {all_targets[0]}")
        config["pool"]["hdg_target_address"] = all_targets[0]
    elif config["pool"]["hdg_target_address"] not in all_targets:
        print(
            f"WARNING: using a pool address which we don't have the private"
            f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
            f"{config['pool']['hdg_target_address']} with {all_targets[0]}"
        )
...
def create_all_ssl(root: Path):
    ...
    hddoge_ca_crt, hddoge_ca_key = get_hddoge_ca_crt_key()
    hddoge_ca_crt_path = ca_dir / "hddoge_ca.crt"
    hddoge_ca_key_path = ca_dir / "hddoge_ca.key"
    hddoge_ca_crt_path.write_bytes(hddoge_ca_crt)
    hddoge_ca_key_path.write_bytes(hddoge_ca_key)
    ...
    hddoge_ca_crt, hddoge_ca_key = get_hddoge_ca_crt_key()
    generate_ssl_for_nodes(ssl_dir, hddoge_ca_crt, hddoge_ca_key, False, overwrite=False)
...
def init(create_certs: Optional[Path], root_path: Path):
    if create_certs is not None:
        if root_path.exists():
            ...
            if hddoge_init(root_path) == 0 and root_path.exists():
                return init(create_certs, root_path)

            print(f"** {root_path} was not created. Exiting **")
            return -1
    else:
        return hddoge_init(root_path)


def hddoge_version_number() -> Tuple[str, str, str, str]:
...
def hddoge_minor_release_number():
    res = int(hddoge_version_number()[2])
    print(f"Install release number: {res}")
    return res


def hddoge_full_version_str() -> str:
    major, minor, patch, dev = hddoge_version_number()
    return f"{major}.{minor}.{patch}{dev}"


def hddoge_init(root_path: Path):
    if os.environ.get("HDDOGE_ROOT", None) is not None:
        print(
            f"warning, your HDDOGE_ROOT is set to {os.environ['HDDOGE_ROOT']}. "
            f"Please unset the environment variable and run hddoge init again\n"
            f"or manually migrate config.yaml"
        )

    print(f"Hddoge directory {root_path}")
    if root_path.is_dir() and Path(root_path / "config" / "config.yaml").exists():
        # This is reached if HDDOGE_ROOT is set, or if user has run hddoge init twice
        # before a new update.
    ...
    check_keys(root_path)
    print("")
    print("To see your keys, run 'hddoge keys show --show-mnemonic-seed'")
