...
from hddoge.rpc.full_node_rpc_client import FullNodeRpcClient
...
from hddoge.util.misc import format_bytes
from hddoge.util.misc import format_minutes
...
async def summary(rpc_port: int, wallet_rpc_port: int, harvester_rpc_port: int, farmer_rpc_port: int) -> None:
    ...
    if amounts is not None:
        print(f"Total hddoge farmed: {amounts['farmed_amount'] / units['hddoge']}")
        print(f"User transaction fees: {amounts['fee_amount'] / units['hddoge']}")
        print(f"Block rewards: {(amounts['farmer_reward_amount'] + amounts['pool_reward_amount']) / units['hddoge']}")
    ...
    if amounts is None:
        if wallet_not_running:
            print("For details on farmed rewards and fees you should run 'hddoge start wallet' and 'hddoge wallet show'")
        elif wallet_not_ready:
            print("For details on farmed rewards and fees you should run 'hddoge wallet show'")
    else:
        print("Note: log into your key using 'hddoge wallet show' to see rewards for each key")
