...
from hddoge.protocols import wallet_protocol
from hddoge.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
from hddoge.protocols.protocol_message_types import ProtocolMessageTypes
from hddoge.protocols.wallet_protocol import (
...
from hddoge.util.byte_types import hexstr_to_bytes
from hddoge.util.errors import Err, ValidationError
from hddoge.util.ints import uint32, uint128
from hddoge.util.keychain import Keychain
from hddoge.util.lru_cache import LRUCache
from hddoge.util.merkle_set import MerkleSet, confirm_included_already_hashed, confirm_not_included_already_hashed
from hddoge.util.path import mkdir, path_from_root
from hddoge.wallet.block_record import HeaderBlockRecord
from hddoge.wallet.derivation_record import DerivationRecord
from hddoge.wallet.settings.settings_objects import BackupInitialized
from hddoge.wallet.transaction_record import TransactionRecord
from hddoge.wallet.util.backup_utils import open_backup_file
from hddoge.wallet.util.wallet_types import WalletType
from hddoge.wallet.wallet_action import WalletAction
from hddoge.wallet.wallet_blockchain import ReceiveBlockResult
from hddoge.wallet.wallet_state_manager import WalletStateManager
...
class WalletNode:
    ...
    def get_key_for_fingerprint(self, fingerprint: Optional[int]) -> Optional[PrivateKey]:
        ...
        if len(private_keys) == 0:
            self.log.warning("No keys present. Create keys with the UI, or with the 'hddoge keys' program.")
            return None
    ...
    async def on_connect(self, peer: WSHddogeConnection):
        if self.wallet_state_manager is None or self.backup_initialized is False:
            return None
    ...
    async def complete_blocks(self, header_blocks: List[HeaderBlock], peer: WSHddogeConnection):
    ...
    async def new_peak_wallet(self, peak: wallet_protocol.NewPeakWallet, peer: WSHddogeConnection):
        if self.wallet_state_manager is None:
            return None
    ...
    async def _sync(self) -> None:
        ...
        peers: List[WSHddogeConnection] = self.server.get_full_node_connections()
        if len(peers) == 0:
    ...
    async def get_additions(self, peer: WSHddogeConnection, block_i, additions) -> Optional[List[Coin]]:
        if len(additions) > 0:
    ...
    async def get_removals(self, peer: WSHddogeConnection, block_i, additions, removals) -> Optional[List[Coin]]:
        assert self.wallet_state_manager is not None
