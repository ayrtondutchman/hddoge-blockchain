from hddoge.protocols import full_node_protocol, introducer_protocol, wallet_protocol
...
from hddoge.util.api_decorators import api_request, peer_required, execute_task
...
class WalletNodeAPI:
    ...
    @peer_required
    @api_request
    async def respond_removals(self, response: wallet_protocol.RespondRemovals, peer: WSHddogeConnection):
        pass

    async def reject_removals_request(self, response: wallet_protocol.RejectRemovalsRequest, peer: WSHddogeConnection):
        """
    ...
    @peer_required
    @api_request
    async def respond_additions(self, response: wallet_protocol.RespondAdditions, peer: WSHddogeConnection):
        pass
    ...
    @peer_required
    @api_request
    async def transaction_ack(self, ack: wallet_protocol.TransactionAck, peer: WSHddogeConnection):
        """
    ...
    async def respond_peers_introducer(
        self, request: introducer_protocol.RespondPeersIntroducer, peer: WSHddogeConnection
    ):
    ...
    @peer_required
    @api_request
    async def respond_peers(self, request: full_node_protocol.RespondPeers, peer: WSHddogeConnection):
        if not self.wallet_node.has_full_node():
