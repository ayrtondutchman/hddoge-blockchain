...
from hddoge.cmds.init_funcs import hddoge_init
from hddoge.daemon.windows_signal import kill
from hddoge.server.server import ssl_context_for_root, ssl_context_for_server
from hddoge.ssl.create_ssl import get_mozilla_ca_crt
from hddoge.util.hddoge_logging import initialize_logging
from hddoge.util.config import load_config
from hddoge.util.json_util import dict_to_json_str
from hddoge.util.path import mkdir
from hddoge.util.service_groups import validate_service
from hddoge.util.setproctitle import setproctitle
from hddoge.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting Hddoge.")
    quit()
...
service_plotter = "hddoge plots create"
...
if getattr(sys, "frozen", False):
    name_map = {
        "hddoge": "hddoge",
        "hddoge_wallet": "start_wallet",
        "hddoge_full_node": "start_full_node",
        "hddoge_harvester": "start_harvester",
        "hddoge_farmer": "start_farmer",
        "hddoge_introducer": "start_introducer",
        "hddoge_timelord": "start_timelord",
        "hddoge_timelord_launcher": "timelord_launcher",
        "hddoge_full_node_simulator": "start_simulator",
    }
...
class WebSocketServer:
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("hddoge daemon exiting in 5 seconds")
...
def launch_plotter(root_path: Path, service_name: str, service_array: List[str], id: str):
    # we need to pass on the possibly altered HDDOGE_ROOT
    os.environ["HDDOGE_ROOT"] = str(root_path)
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service with HDDOGE_ROOT: {os.environ['HDDOGE_ROOT']}")
...
async def async_run_daemon(root_path: Path) -> int:
    hddoge_init(root_path)
    config = load_config(root_path, "config.yaml")
    setproctitle("hddoge_daemon")
...
def main() -> int:
    from hddoge.util.default_root import DEFAULT_ROOT_PATH
